...
def mu2f(miu,omega,J): #将跃迁矩阵元转换为f值，miu输入应该为国际单位制，所以一般来说是1.5*ea0的形式，数量级很小，下同。omega为圆频率
    f = 2 * constants.me * omega/(3*constants.e**2 * constants.hbar) * miu**2/(2*J+1) #除以下态简并度
    return f

def f2mu(f, omega,j): #将f转换为跃迁矩阵元
    miu = ((f *(2*j+1) *3 * constants.e**2 * constants.hbar)/(2 * constants.me * omega))**0.5
    return miu

def csv_mu2f(csvfile,jg,jf):
    # 读取Hgwrong.csv,将gf的值用mu2f转换一下，然后再新建一列“gf_f”写入Hgwrong.csv,jg和jf分别是g、f能级角动量
    Hgwrong = pd.read_csv(csvfile)
    Hgwrong["gf_f"] = Hgwrong.apply(lambda x: mu2f(x["gf"]*constants.ea0,x["f_energy_Hz"]*2*constants.Pi,jg),axis=1)
    Hgwrong["fb_f"] = Hgwrong.apply(lambda x: mu2f(x["fb"]*constants.ea0,x["f_energy_Hz"]*2*constants.Pi,jf),axis=1)
...
def Kaidraw1987(elelist):
    ...
    plt.grid(True)
    # 加标题
    plt.title('Kai of Hg(Prof Yu) and Hg2(1986)')
    plt.show()
...
def MultiIsotopeS(omega,gamma_gb,delta_wp,isotopeinfo,ExpSynsDict,Pool): #多同位素S(w1+w2)因子
    ...
    for i in range(len(isotopeinfo)):
        ...
        #print("同位素{}多普勒展宽为{}".format(isotopeinfo["isotope"][i],omegaD))
...
def FWMout(P1, P2, P3, wavnum1, wavnum4, ExpSynsDict,Isotope,type, Pool:FWMpool,log = False,test = False):
    #关闭打印
    if log== False:
    ...
    curve = np.sin(ytrue* L_in_cm /2) / (ytrue * L_in_cm /2) 
    ...
    return p4parallel,p4parallelperfect,p4tight,compensatetheta
